...
namespace com.xrpractices.curvedui
{
    ...
    {
        public static Mesh GenerateMesh(int resolution, float radius, float height,
            float chordangle)
        {
            Mesh mesh = new Mesh();
            ...
            int triIndex = 0;
            Vector2[] uv = (mesh.uv.Length == vertices.Length)
                ? mesh.uv
                : new Vector2[vertices.Length];
            float angleStep = chordangle / resolution;
